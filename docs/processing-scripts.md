...
#
# Note: These variables are documented in the StartLiveData algorithm:
# https://docs.mantidproject.org/nightly/algorithms/StartLiveData-v1.html
...
**Note**: These workspaces are documented in the [StartLiveData algorithm documentation](https://docs.mantidproject.org/nightly/algorithms/StartLiveData-v1.html).
...
Before deploying, test with the [StartLiveData algorithm](https://docs.mantidproject.org/nightly/algorithms/StartLiveData-v1.html) to verify that the script logic is correct.

Starting a test server can verify that permissions are right, but that isn't nearly as likely a place for issues as the script itself.
...
When memory exceeds the limit, the daemon automatically restarts processing. This will be shown in the logs.

### Monitoring Memory

Debugging memory issues is easier in MantidWorkbench with StartLiveData.
...
```python
# Get run number (single line with check)
run_number = accumulation.getRun().getProperty("run_number").value
if run_number < 1:
    return  # in between runs
