...
```
Command `gh release create` both creates tag `v1.20` and a GitHub release accessible at
...
- `$HOME/rpmbuild/SRPMS/python-livereduce-watchdog-<version>-1.el9.noarch.rpm`
...
For RPM development and testing, visit the [RPM testing guide](../test/rpm/README.md).

The last step requires manually uploading these packages to the GitHub release page created in the previous step.
You will need to edit the release page and `Attach binaries by dropping them here or selecting them`.
