...
```mermaid
flowchart LR
    Server["Terminal 1<br/>Fake Server<br/>Sends chunks"]
    LiveReduce["Terminal 2<br/>Live Reduce<br/>Process Data<br/>Writes output"]
    Monitor["Terminal 3<br/>Monitor Logs<br/>tail -f output"]

    Server -->|network| LiveReduce
    LiveReduce -->|writes| Monitor
    Monitor -.->|see results| Server
```
...
**Note**: This section demonstrates interactive usage patterns. For comprehensive documentation about the `StartLiveData` algorithm, variables, and workspaces, see the [StartLiveData algorithm documentation](https://docs.mantidproject.org/nightly/algorithms/StartLiveData-v1.html).
