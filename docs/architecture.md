...
- **Add**: Sum processed chunks together (default, good for event data)
- **Replace**: Each chunk replaces previous data (good for histogram monitoring)
- **Append**: Stack spectra from each chunk (workspace grows)

### 5. Post-Processing Executes
- Every `UpdateEvery` seconds (default 30)
...
- Restarts processing if limits exceeded
...
The two-stage model separates lightweight per-chunk operations from heavier accumulated data analysis:

### Processing Script (`reduce_<INSTRUMENT>_live_proc.py`)

**Purpose**: Fast, lightweight operations on each incoming chunk

**When it runs**: Every time a chunk arrives (unpredictable timing)
...
**When it runs**: Every `UpdateEvery` seconds (default 30s)
...
- Use post-processing for database updates
